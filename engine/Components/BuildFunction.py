...
# START HERE
def build_statements(from_statement, elements, variables):
    ...
    print("calling parse()")
    returned_str = parse(elements)

    # value = check_brackets(test_check_brackets)
    # value = check_brackets(elements)
    # print(value)
...
def find_connective(arr):
    ...
    for i in range(len(arr)):
        ...
        if elem in left_brackets:
            inBracket = True
            count += 1
        if elem in right_brackets and inBracket:
            if count == 1:
                inBracket = False
            count -= 1
        if elem in binary_connectives and not inBracket and count == 0:
            print("debug")
            return elem, i
